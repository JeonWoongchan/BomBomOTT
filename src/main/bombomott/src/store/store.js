...
const nowProfile = createSlice({ //현재 유저가 이용중인 프로필코드
  name: "nowProfile",
  initialState: '1',
  reducers: {
...
const nowProfileData = createSlice({ //현재 유저가 이용중인 프로필 데이터
  name: "nowProfileData",
  initialState: '',
  reducers: {
      setNowProfileData: (state, action) => {
      return action.payload;
      },
  }
});

export const { setNowProfileData } = nowProfileData.actions;
...
export default configureStore({
  reducer: {
    ...
    nowProfileData: nowProfileData.reducer,
