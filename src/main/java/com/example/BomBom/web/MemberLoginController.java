...
public class MemberLoginController {
    ...
    public ResponseEntity<Map<String,Object>> PasswordCheck(@RequestBody MemberLoginDto dto, @ModelAttribute Member member, HttpSession session) {
        ...
        if (sql == null) {
            responseMap.put("status", 11); // 다시돌아가
            return ResponseEntity.ok(responseMap);
        } else {
            dto.setEmail(sql);
    ...
    public ResponseEntity<Map<String,Object>> payment(@RequestBody MemberUpdateDto dto,
                                                            @ModelAttribute Member member,
                                                            HttpSession session) {
        Map<String, Object> responseMap = new HashMap<>();
        String email = (String) session.getAttribute("email");
    ...
    public ResponseEntity<Map<String,Object>> selectProfile(@RequestBody Profile profile ,HttpSession session) {
        ...
        int count = profileService.profileCount(id);

        if(count == 0 ) {
            responseMap.put("status", -1);
            return ResponseEntity.ok(responseMap);
        } else {
            List<Profile> profiles = profileService.findProfile(id);
            responseMap.put("status", 1);
